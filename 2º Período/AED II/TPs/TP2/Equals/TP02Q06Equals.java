...
class TP02Q06Equals{
    public static boolean ehIgual(String s1, String s2){
	    return ehIgual(s1,s2,0); 
    }
    public static boolean ehIgual(String s1, String s2,int i){
    ...
    public static boolean ehIgualNoCase(String s1, String s2){
         ...
         return ehIgualNoCase(s1,s2,0);
    }
    
        //Método que verifica se a String é igual a outra sem capitalização
    public static boolean ehIgualNoCase(String s1, String s2,int i){
        boolean ehigual = true;
	    if(s1.length() != s2.length()){
            ehigual = false;
        }
        if(i>s1.length()-1){
	        ...
	        if(s1.charAt(i) != s2.charAt(i)){
                ...
                ehigual = ehIgualNoCase(s1,s2,i+1);         
            }
        }        
	    return ehigual;	
   }
