...
class Fila {
  ...
  /**
   ...
   public Pessoa remover() throws Exception {
      ...
      Pessoa resp = array[primeiro].clone();
   ...
   public boolean isVazia() {
    ...
    /**
     * Realiza a média das idades das pessoas presentes na fila
     * @return double resposta média idades
     */
    public double mediaIdades()throws Exception{
        ...
        double resposta=0;
        double cont = 0.0;
        // Divisão da soma das idades, pela quantidade de pessoas. (De modo circular)
        for(int i = primeiro; i!= ultimo; i = (i+1) % array.length){
            resposta += (double)array[i].getIdade();
            cont++;
        }        
        resposta /= cont;
        return resposta;
