/**
 * TP05Q01 Ordenação Quicksort por ano de nascimento.
...
class Lista {
    ...
    * @param int dir fim do array a ser ordenado
    ...
    private void quicksort(int esq, int dir) {
        int i = esq, j = dir;
        int pivo = array[(dir+esq)/2].getAno();
        int pivoId = array[(dir+esq)/2].getId();
            while (i <= j) {
            while ((array[i].getAno() < pivo) || (array[i].getAno() == pivo && array[i].getId() < pivoId)){
                 i++;
		comparacoes++;
            }
            while ((array[j].getAno() > pivo) || (array[j].getAno() == pivo && array[j].getId() > pivoId)){
                 j--;
		comparacoes++;
            }
            if (i <= j) {
