/**
 * TP05Q01 Ordenação Mergesort por data da morte.
...
class Lista {
 ...
 /**
  * O método mergesort foi realizado com auxílio e instruções do aluno Luigi Domênico.
  */
	...
	/**
         * Ordenação Mergesort
	 * @param inicio int inicio do array
	 * @param fim int fim do array.
         */	
	public void mergesort(int inicio, int fim){
		if(inicio<fim){
			int meio = (inicio+fim)/2;
			//Subdivisões do array em elementos até subconjunto de apenas 1 elemento.
     ...
     * @param comeco int começo do subconjunto
     * @param meio int meio do subconjunto
     * @param fim int fim do subconjunto
     */
	public void mergesort(int inicio,int meio, int fim){
		int tamanho = fim - inicio+1;
		Pessoa[] tmp = new Pessoa[tamanho]; //Array para ordenação
		int i = inicio, j = meio+1,k=0;
		boolean fimEsq = false, fimDir = false;
		while(k<tamanho){
		   if(fimEsq == false && fimDir == false){
			if(array[i].getMorte().compareTo(array[j].getMorte())>0 ||((array[i].getMorte().equals(array[j].getMorte()) && array[j].getId() < array[i].getId()))){
				tmp[k] = array[j++].clone();
				comparacoes++;
				movimentacoes++;
			}else{
		...
		// Realiza a transferencias do array tmp ordenado para o array inicial.
		for(int x = inicio, l =0; x <= fim; l++,x++){
			array[x] = tmp[l].clone();
			movimentacoes++;
		}
	}
