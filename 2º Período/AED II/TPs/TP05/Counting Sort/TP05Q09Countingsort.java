/**
 * TP05Q01 Ordenação CoutingSort por ano de nascimento.
...
class TP05Q09Countingsort{
	...
	Arq.openWrite("matrícula_coutingsort.txt", "UTF-8");
	Arq.print("569631\t");	
...
class Lista {
    ...
    public void countingsort() {
        //Array para contar o numero de ocorrencias de cada elemento (ano de nascimento)
        int[] count = new int[getMaior()*2 + 1];
        ...
        /**
         * Solução desenvolvida a partir de dicas e comentários do aluno Luigi Domênico.
         */
        //Agora, o count[i] contem o numero de elemento iguais a i
        for (int i = 0; i < n;i++){
            count[array[i].getAno() + getMaior()]++;
	    movimentacoes++;
        }
        ...
        for(int i = getN()-1; i >= 0;i--){
            ordenado[count[array[i].getAno()+getMaior()] -1] = array[i].clone();
            count[array[i].getAno()+getMaior()]--;
	    movimentacoes++;
            
        }
        
        //Pessoas com o ano de Nascimento Igual
        for(int i = 0; i<getN()-1;i++){
            Pessoa aux;
            int anoDireita = ordenado[i+1].getAno();
            int anoEsquerda = ordenado[i].getAno();
            if(anoEsquerda == anoDireita){
                if(ordenado[i+1].getId() <ordenado[i].getId()){
                    aux = ordenado[i+1];
