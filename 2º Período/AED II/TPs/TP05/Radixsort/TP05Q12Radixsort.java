...
class Lista {
 ...
 /**
  * O método Radix foi realizado com auxílio e instrução do aluno Luigi Domênico
  */
	...
	public void radixsort(){
		int maior = getMaiorDoidao(),exp =1;
		Pessoa[] emordem = new Pessoa[getN()];
		while(maior/exp >0){
		   //Contagem de -9 à 10
		   int[] count = new int[20];
		   // Count[i] com elementos iguais a i.
		   for(int i =0; i<getN();i++){
			    int doidao = array[i].getDoidao();
			    if(doidao < 0){
			        count[9 - ((doidao/exp) % 10)]++;
			        movimentacoes++;
			        comparacoes++;
			    } else {
			        count[10 + ((doidao/exp) % 10)]++;
			        movimentacoes++;
			    }
		   }
		   //Count[i] acumulativo.
		   ...
		   // Ordenação pelo digito
	       for(int i = getN()-1;i>=0; i--){
			    int doidao = array[i].getDoidao();
			    if(doidao < 0){
				    emordem[--count[9 - ((doidao/exp) % 10)]] = array[i].clone();
				    movimentacoes++;
				    comparacoes++;
			    } else{
				    emordem[--count[10+((doidao/exp) % 10)]] = array[i].clone();
		        }
           }    
		   // Traferindo apra array inicial.
		   for(int i =0; i<getN();array[i] = emordem[i].clone(),i++){
			movimentacoes++;
		   }
		   // Próximo dígito da chave de entrada
		   exp *= 10;
		}
	}

	/**
	 * Método para recuperar o maior elemento da chave de pesquisa
         * @return maior elemento doidao (Ano da morte*1000+id)
 	 */

	public int getMaiorDoidao(){
		int maior = 0;
		for(int i =0; i<getN(); i++){
			if(array[i].getDoidao() > maior){
				maior = array[i].getDoidao();
			}
		}
		return maior;
	}
